// Command simplestonks-qt is the Qt (miqt) entrypoint for the simpleStonks
// stock tracker. It bootstraps the same config store, logger, and quote
// provider as the Fyne entrypoint, then runs the Qt UI.
package main
...
func main() {
	...
	slog.Info("simplestonks (qt) starting", "symbols", len(store.Get().Symbols))
